...
do
    ...
    local function ApplyQueryReplacements(mode, query, moduleName)
        if (!Replacements[moduleName]) then
            return query
        end
    
        local result = query
        local entries = Replacements[moduleName][mode]
    ...
    local function BuildSelectQuery(queryObj)
        ...
        -- it seems gmod's sqlite doesn't support limit statement huh
    ...
    local function BuildInsertQuery(queryObj, bIgnore)
        local suffix = (bIgnore and (queryObj.database.module == "sqlite" and "INSERT OR IGNORE INTO" or "INSERT IGNORE INTO") or "INSERT INTO")
    ...
    local function BuildDeleteQuery(queryObj)
        ...
        if (isnumber(queryObj.limit) and queryObj.database.module ~= 'sqlite') then
            queryString[#queryString + 1] = " LIMIT "
            queryString[#queryString + 1] = queryObj.limit
        end
    ...
    local function BuildCreateQuery(queryObj)
        ...
        if (istable(queryObj.createList) and #queryObj.createList > 0) then
            ...
            for i = 1, #queryObj.createList do
                if (queryObj.database.module == "sqlite") then
                    createList[#createList + 1] = queryObj.createList[i][1].." "..ApplyQueryReplacements("Create", queryObj.createList[i][2], queryObj.database.module)
                else
    ...
    local function BuildAlterQuery(queryObj)
        ...
        if (istable(queryObj.add)) then
            queryString[#queryString + 1] = " ADD "..queryObj.add[1].." "..ApplyQueryReplacements("Create", queryObj.add[2], queryObj.database.module)
    ...
    function QUERY_CLASS:Execute(bQueueQuery)
        ...
        if (isstring(queryString)) then
            if (!bQueueQuery) then
                return self.database:RawQuery(queryString, self.callback)
            else
                return self.database:Queue(queryString, self.callback)
            end
        end
    end
end
...
DATABASE.__tostring = function(self)
    if (self.module == 'mysqloo') then
        return (self.m_Schema .. "@" .. self.m_Hostname)
    else
        return 'sqlite'
    end
end

AccessorFunc(DATABASE, 'm_Name', 'Name')
AccessorFunc(DATABASE, 'm_Schema', 'Schema')
AccessorFunc(DATABASE, 'm_Hostname', 'Hostname')

function DATABASE:Select(tableName)
	return QUERY_CLASS:New(tableName, "SELECT", self)
end

function DATABASE:Insert(tableName)
	return QUERY_CLASS:New(tableName, "INSERT", self)
end

function DATABASE:InsertIgnore(tableName)
	return QUERY_CLASS:New(tableName, "INSERT IGNORE", self)
end

function DATABASE:Update(tableName)
	return QUERY_CLASS:New(tableName, "UPDATE", self)
end

function DATABASE:Delete(tableName)
	return QUERY_CLASS:New(tableName, "DELETE", self)
end

function DATABASE:Drop(tableName)
...
function DATABASE:Create(tableName)
	return QUERY_CLASS:New(tableName, "CREATE", self)
end

function DATABASE:Alter(tableName)
...
function DATABASE:Connect(host, username, password, database, port, socket, flags)
	port = port or 3306
...
function DATABASE:RawQuery(query, callback, flags, ...)
	if (self.module == "mysqloo") then
...
function DATABASE:Queue(queryString, callback)
	if (isstring(queryString)) then
        table.insert(self.queue, {queryString, callback})
	end
end
...
function DATABASE:SetModule(moduleName)
...
function DATABASE:OnConnected()
	...
	hook.Run("onyx.DatabaseConnected", self:GetName())
end

-- Called when the database connection fails.
function DATABASE:OnConnectionFailed(errorText)
	self:Error(string.format("[mysql] Unable to connect to the database!\n%s\n", errorText))

	hook.Run("onyx.DatabaseConnectionFailed", self:GetName(), errorText)
end

function DATABASE:GetName()
    return ('(' .. (self.m_Name or 'gsql') .. ')')
end
...
do    
    ...
    function DATABASE:Log(text)
        MsgC(Color(200, 200, 0), self:GetName() .. " ", colorWhite, tostring(self), " -> ", text, "\n")
    end
    
    function DATABASE:Error(text)
        MsgC(Color(200, 200, 0), self:GetName() .. " ", colorError, "[ERROR] ", colorWhite, tostring(self), " -> ", text, "\n")
    end
    
    function DATABASE:Success(text)
        MsgC(Color(200, 200, 0), self:GetName() .. " ", colorSuccess, "[SUCCESS] ", colorWhite, tostring(self), " -> ", text, "\n")
    end
end
...
function onyx.sql.Create(moduleName, name, data)
    assert(moduleName, 'missing module name')
    assert(name, 'missing database name')
    assert(data, 'missing data for connecting')
    ...
    db:SetModule(moduleName)
    db:Connect(data.hostname, data.username, data.password, data.database, data.port, data.socket, data.flags)

    timer.Create('onyx.sql.QueueProcessor_' .. name, .1, 0, function()
        local success, errorString = pcall(db.Think, db)
        if (not success) then
            db:Error('Error via query: ' .. errorString)
        end
    end)
