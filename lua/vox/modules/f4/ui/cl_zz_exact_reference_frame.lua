...
local C={bg=Color(5,14,28,245),panel=Color(8,25,48,238),card=Color(11,32,60,232),card2=Color(13,39,72,232),border=Color(0,174,255,80),accent=Color(0,174,255),green=Color(35,225,120),red=Color(255,75,95),amber=Color(255,190,65),text=Color(240,248,255),soft=Color(145,172,200)}
...
function PANEL:BuildSidebar()
    ...
    for _,t in ipairs(self.tabs) do
        ...
        b.Paint=function(p,w,h)
            ...
            if p:IsHovered() then surface.SetDrawColor(ColorAlpha(C.accent,80)); surface.DrawOutlinedRect(0,0,w,h,1) end
...
local function addCard(parent,x,y,w,h,title,value,sub,col)
    local p=parent:Add('Panel'); p:SetPos(x,y); p:SetSize(w,h); p.Paint=function(_,cw,ch)
        glass(0,0,cw,ch,8,col or C.border); draw.SimpleText(string.upper(title),'VoxRef.Tiny',14,12,C.soft,0,0); draw.SimpleText(value,'VoxRef.CardTitle',14,34,C.text,0,0); draw.SimpleText(sub or '', 'VoxRef.Small',14,62,col or C.green,0,0)
    end; return p
end
...
function PANEL:ListPanel(parent,x,y,w,h,title,rows)
    ...
    local yy=34
    for _,r in ipairs(rows) do local row=p:Add('Panel'); row:SetPos(10,yy); row:SetSize(w-20,30); yy=yy+36; row.Paint=function(_,rw,rh) rr(0,0,rw,rh,6,Color(12,35,65,210)); draw.SimpleText(r[1],'VoxRef.Small',12,7,C.text,0,0); draw.SimpleText(r[2] or '','VoxRef.Tiny',12,20,C.soft,0,0) end end
end
