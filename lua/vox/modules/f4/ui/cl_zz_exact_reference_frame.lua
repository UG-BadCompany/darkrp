...
function PANEL:Init()
    vox.f4.frame=self
    self:SetSize(math.min(ScrW()*0.92,1024), math.min(ScrH()*0.86,576)); self:Center(); self:SetTitle('')
...
local function addCard(parent,x,y,w,h,title,value,sub,col)
    local p=parent:Add('Panel'); p:SetPos(x,y); p:SetSize(w,h); p.Paint=function(_,cw,ch)
        softCard(0,0,cw,ch,8,Color(10,27,57,224)); draw.SimpleText(string.upper(title),'VoxRef.Tiny',14,10,C.soft,0,0); draw.SimpleText(value,'VoxRef.CardTitle',14,31,C.text,0,0); draw.SimpleText(sub or '', 'VoxRef.Tiny',14,58,col or C.green,0,0)
    end; return p
end
function PANEL:BuildContent()
    ...
    local search=c:Add('DTextEntry'); search:SetPos(18,14); search:SetSize(c:GetWide()-36,30); search:SetText(''); search:SetPlaceholderText('Search the menu...')
...
function PANEL:BuildDashboard(c)
    ...
    addCard(c,18,78,150,66,'Wallet',money(moneyVal),'Bank Balance',C.green)
    addCard(c,178,78,150,66,'Current Job',IsValid(lp) and (lp:getDarkRPVar('job') or 'Citizen') or 'Citizen','View Jobs →',C.accent)
    addCard(c,338,78,150,66,'Players Online',#player.GetAll()..' / 64','Join the community',C.accent)
    addCard(c,498,78,150,66,'Server Time',os.date('%H:%M'),'Today',C.accent)
    self:ListPanel(c,18,160,300,112,'ANNOUNCEMENTS',{{'Welcome to Vox City','Make sure to read the rules'},{'Double XP Weekend','Enjoy 2x XP on all jobs'},{'Update v1.0.5','View changelog on Discord'}})
    self:ListPanel(c,330,160,230,112,'POPULAR JOBS',{{'Police Officer','$75 / min      8/10'},{'Medic','$85 / min      3/6'},{'SWAT','$95 / min      2/4'}})
    self:ListPanel(c,572,160,180,112,'QUICK ACTIONS',{{'Laws of the Land',''},{'Wanted Players',''},{'Report Player',''},{'Open Inventory',''}})
    self:ListPanel(c,18,286,300,112,'STAFF ONLINE',{{'superadmin','Owner'},{'Voxberg','Administrator'}})
    self:ListPanel(c,330,286,422,112,'WANTED PLAYERS',{{'John Wick','★★★★★        $5,000'},{'Tony Montana','★★★★☆        $2,500'}})
end
function PANEL:ListPanel(parent,x,y,w,h,title,rows)
    ...
    local yy=28
    for _,r in ipairs(rows) do local row=p:Add('Panel'); row:SetPos(10,yy); row:SetSize(w-20,24); yy=yy+28; row.Paint=function(_,rw,rh) rr(0,0,rw,rh,5,Color(13,35,68,210)); draw.SimpleText(r[1],'VoxRef.Tiny',12,5,C.text,0,0); draw.SimpleText(r[2] or '','VoxRef.Tiny',12,15,C.soft,0,0) end end
end
