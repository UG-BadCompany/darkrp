...
do
    local q = vox.f4.db:Create('vox_f4_object_stats')
        ...
        q:Callback(function()
            vox.f4:Print('Initialized stats database table.')
        end)
    q:Execute()
end
...
local function clearOldRecords()
    ...
    vox.f4.db:Queue(string.format([[
        DELETE FROM `vox_f4_object_stats`
            WHERE `unixDate` <= %d;
    ]], oldUnixDate), function()
        vox.f4:Print('Cleared old records')
    end)
end
hook.Add('InitPostEntity', 'vox.f4.ClearOldStats', clearOldRecords)
...
local function addObjectRecord(objectType, objectID, toAddAmount)
    ...
    local q = vox.f4.db:Select('vox_f4_object_stats')
        ...
        q:Callback(function(result)
            ...
            if (hasRecord) then
                ...
                local q = vox.f4.db:Update('vox_f4_object_stats')
                ...
                local q = vox.f4.db:Insert('vox_f4_object_stats')
...
hook.Add('OnPlayerChangedTeam', 'vox.f4.stats', function(ply, oldTeam, newTeam)
...
do
    ...
    for objectID, hookName in pairs(OBJECTS) do
        hook.Add(hookName, 'vox.f4.stats', function(ply, item)
...
timer.Create('vox.f4.ProcessStatsQueue', 1, 0, function()
...
local function fetchObjectsData(objectType, timeRange, callback)
    ...
    vox.f4.db:Queue(string.format([[
        SELECT objectID, SUM(amount) as amount FROM `vox_f4_object_stats`
            WHERE `unixDate` >= %d AND `objectType` = '%s'
            GROUP BY `objectID`;
    ]], minRange, vox.f4.db:Escape(objectType)), function(result)
        callback(result or {})
    end)
end

net.Receive('vox.f4:RequestStats', function(len, ply)
    if ((ply.vox_f4_NextNetRequest or 0) > CurTime()) then
        return
    end

    ply.vox_f4_NextNetRequest = CurTime() + .33
    ...
    CAMI.PlayerHasAccess(ply, 'vox_f4_edit', function(bHasAccess)
        if (bHasAccess) then
            fetchObjectsData(objectType, timeRange, function(result)
                if (IsValid(ply)) then
                    netchunk.Send(ply, 'vox.f4:SendStats', {
                        timeID = timeID,
            ...
            -- increase delay
            ply.vox_f4_NextNetRequest = CurTime() + 1
        end
    end)
end)
