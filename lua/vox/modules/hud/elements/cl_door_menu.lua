...
local function openDoorMenu( client, setDoorOwnerAccess, canChangeSettings )
    ...
    if ( canChangeSettings and ( isOwned or isNonOwnable or doorGroup or hasTeams ) or isClientOwner ) then
        ...
        insert( title, function()
            local frame = vox.SimpleQuery( title, L( 'door_title_help' ), true, function( value )
            ...
            frame.textEntry:SetColorHover( vox.OffsetColor( colors.primary, -10 ) )
            ...
            frame.textEntry.currentOutlineColor = vox.CopyColor( colors.secondary )
        end, 'door_title' )
    end
    ...
    elseif ( amount > 0 and not IsValid( doorFrame ) ) then
        local size = vox.hud.ScaleTall( 512 )

        local choiceWheel = vgui.Create( 'vox.hud.ChoiceWheel' )
        ...
        choiceWheel:AddChoice( { name = vox.lang:Get( 'close' ), wimgID = 'radial_close' } )
...
vox.hud.OverrideGamemode( 'vox.hud.OverrideKeysMenu', overrideDarkRP )
