...
local function drawMainHUD( self, client, scrW, scrH )
    ...
    -- Premium 2026 reference HUD: compact glass card, round avatar, economy cluster and smooth bars.
    do
        local t = vox.ThemeTokens and vox.ThemeTokens() or {}
        local accent = colors.accent or t.accent or Color( 0, 174, 255 )
        local cardW = hud.ScaleWide( 360 )
        local cardH = hud.ScaleTall( hasHunger and 174 or 154 )
        local cardX = space
        local cardY = scrH - cardH - space
        local inner = hud.ScaleTall( 14 )
        local avatarSize = hud.ScaleTall( 64 )
        local contentX = cardX + inner + avatarSize + hud.ScaleWide( 14 )
        local contentW = cardW - ( contentX - cardX ) - inner
        local jobText = client:getDarkRPVar( 'job' ) or team.GetName( client:Team() )

        vox.ui.DrawGlassPanel( cardX, cardY, cardW, cardH, { radius = 18, accent = accent, alpha = 232, glow = 30 } )
        vox.ui.DrawAccentLine( cardX + inner, cardY + inner, hud.ScaleWide( 42 ), hud.ScaleTall( 3 ), accent )
        vox.ui.DrawStatusDot( cardX + cardW - inner - hud.ScaleTall( 5 ), cardY + inner + hud.ScaleTall( 5 ), hud.ScaleTall( 4 ), colors.positive or t.success or Color( 42, 220, 120 ), true )

        local avatarX = cardX + inner
        local avatarY = cardY + inner + hud.ScaleTall( 8 )
        local avatarRadius = avatarSize * .5
        if ( IsValid( self.AvatarPanel ) ) then
            local mask = vox.CalculateCircle( avatarX + avatarRadius, avatarY + avatarRadius, avatarRadius, 42 )
            ...
            vox.ui.DrawAvatarBadge( avatarX, avatarY, avatarSize, teamColor )
        end
        vox.DrawOutlinedCircle( avatarX + avatarRadius, avatarY + avatarRadius, avatarRadius + 1, hud.ScaleTall( 2 ), teamColor )

        draw.SimpleText( name, hud.fonts.SmallBold, contentX, cardY + inner + hud.ScaleTall( 6 ), colorTextPrimary, 0, 0 )
        draw.SimpleText( jobText, hud.fonts.TinyBold, contentX, cardY + inner + hud.ScaleTall( 28 ), teamColor, 0, 0 )

        if ( hud:GetOptionValue( 'display_money' ) ) then
            draw.SimpleText( moneyFormatted, hud.fonts.SmallBold, contentX, cardY + inner + hud.ScaleTall( 54 ), colors.money or colors.positive, 0, 0 )
            if ( hud:GetOptionValue( 'display_salary' ) ) then
                draw.SimpleText( salaryFormatted, hud.fonts.ExtraTinyBold, cardX + cardW - inner, cardY + inner + hud.ScaleTall( 58 ), colors.money or colors.positive, 2, 0 )
            end
        end

        local function drawPremiumBar( row, label, fraction, barColor )
            local barX = contentX
            local barY = cardY + inner + hud.ScaleTall( 84 + row * 21 )
            local barH = hud.ScaleTall( 7 )
            draw.SimpleText( label, hud.fonts.ExtraTinyBold, barX, barY - hud.ScaleTall( 4 ), colorTextSecondary, 0, 4 )
            vox.ui.DrawProgressBar( barX + hud.ScaleWide( 58 ), barY, contentW - hud.ScaleWide( 98 ), barH, fraction, barColor, ColorAlpha( colorTextPrimary, 22 ) )
            draw.SimpleText( math.Round( fraction * 100 ) .. '%', hud.fonts.ExtraTinyBold, cardX + cardW - inner, barY + barH * .5, colorTextPrimary, 2, 1 )
        end

        if ( hud:GetOptionValue( 'display_health' ) ) then
            drawPremiumBar( 0, 'Health', lerpHealth, colors.negative or t.health or Color( 255, 70, 90 ) )
        end
        drawPremiumBar( 1, 'Armor', lerpArmor, colors.armor or t.armor or Color( 70, 135, 255 ) )
        if ( hasHunger ) then
            ...
            drawPremiumBar( 2, 'Hunger', lerpHunger, colors.hunger or t.hunger or Color( 255, 190, 75 ) )
        end

        if ( vox.hud:GetOptionValue( 'display_level' ) and vox.hud.IsLevellingEnabled() ) then
            local level, xp, maxXP = vox.hud.GetLevelData( client )
            local xpY = cardY + cardH - inner - hud.ScaleTall( 13 )
            draw.SimpleText( 'Level ' .. tostring( level ), hud.fonts.ExtraTinyBold, cardX + inner, xpY, colorTextSecondary, 0, 1 )
            vox.ui.DrawProgressBar( contentX, xpY - hud.ScaleTall( 3 ), contentW, hud.ScaleTall( 5 ), math.Clamp( xp / math.max( maxXP, 1 ), 0, 1 ), colors.xp or colors.hunger, ColorAlpha( colorTextPrimary, 18 ) )
        end
