...
local WIMG_LOCKDOWN = vox.wimg.Create( 'hud_lockdown', 'smooth mips' )
local WIMG_WANTED = vox.wimg.Create( 'hud_wanted', 'smooth mips' )
local WIMG_ARRESTED = vox.wimg.Create( 'hud_arrested', 'smooth mips' )
...
hud:RegisterElement( 'status', {
    ...
    drawFn = function( element, client, scrW, scrH )
        local parent = vox.hud.elements[ 'agenda' ]
...
vox.hud.OverrideGamemode( 'vox.hud.OverrideArrest', function()
