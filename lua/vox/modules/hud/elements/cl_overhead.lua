...
local CONVAR_MAX_DETAILED = CreateClientConVar( 'cl_vox_hud_3d2d_max_details', '3', true, false, '', 1, 5 )
...
local FONT_NAME = vox.hud.CreateFont3D2D( 'OverheadName', 'Comfortaa Bold', 72 )
local FONT_JOB = vox.hud.CreateFont3D2D( 'OverheadJob', 'Comfortaa SemiBold', 40 )
local FONT_STATUS = vox.hud.CreateFont3D2D( 'OverheadStatus', 'Comfortaa Bold', 64 )

local drawShadowText = vox.hud.DrawShadowText

local getStatuses, hasStatus do
    local function createStatus( data )
        data.wimg = vox.wimg.Create( data.icon, 'smooth mips' )
    ...
    createStatus( {
        ...
        getColor = function()
            ...
            local color = vox.LerpColor( fraction, COLOR_RED, COLOR_BLUE )
            return color
        end
    } )
...
local function drawStatus( ply, y )
    ...
    for i = 1, amount do
        ...
        if ( isSingle and status.big ) then
            status.text = status.text or vox.lang:Get( 'hud_status_' .. status.id )
...
local function drawInfo( ply, client )
    ...
    if ( shouldDrawHealth ) then
        ...
        local healthColor = vox.LerpColor( healthFraction, COLOR_LOW_HP, COLOR_MAX_HP )
...
timer.Create( 'vox.hud.CollectNearestPlayers', 1 / 10, 0, function()
...
hook.Add( 'PostDrawTranslucentRenderables', 'vox.hud.DrawOverheadInfo', function()
    local client = LocalPlayer()
    local index = 0
    ...
    for _, object in ipairs( nearest ) do
        ...
        if ( IsValid( ply ) ) then
            ...
            if ( shouldDraw ) then
                ...
                local heightOffset = jobTable.voxOverheadOffset or ( maxs.z + 10 )
