...
local CONVAR_BLUR = CreateClientConVar( 'cl_vox_hud_speedometer_blur', '1', true, false, '', 0, 1 )
...
local function drawFuelHUD( element, client, vehicle, scrW, scrH, theme )
    ...
    draw.SimpleText( vox.lang:Get( 'fuel' ), hud.fonts.TinyBold, x + horPadding, textY, colors.textPrimary, 0, 0 )
...
local function drawVehicleHUD( element, client, scrW, scrH )
    ...
    cache.outerMask = cache.outerMask or vox.CalculateCircle( x0, y0, outerRadius + 1, 24 )
    cache.innerMask = cache.innerMask or vox.CalculateArc( x0, y0, 180 + 45, arcLength, outerRadius + 1, 24, true )
    ...
    local colorLine = vox.LerpColor( circFraction, COLOR_GREEN, COLOR_RED )

    -- Draw blur
    if ( CONVAR_BLUR:GetBool() ) then
        vox.DrawWithPolyMask( outerMask, function()
            vox.DrawBlurExpensive( vgui.GetWorldPanel(), 6 )
        end )
    end

    -- Draw background
    vox.DrawCircle( x0, y0, outerRadius, ColorAlpha( colors.primary, theme.dark and 200 or 100 ) )
    ...
    vox.DrawWithPolyMask( innerMask, function()
        vox.DrawOutlinedCircle( x0, y0, innerRadius, 1, colors.textSecondary )
    end )

    -- Draw line fill
    local innerMaskFill = vox.CalculateArc( x0, y0, 180 + 45, arcLength * circFraction, innerRadius, 24, true )

    vox.DrawWithPolyMask( innerMaskFill, function()
        vox.DrawOutlinedCircle( x0, y0, innerRadius, 2, colorLine )
    end )
    ...
    if ( isAdvanced ) then
        ...
        if ( statusTable ) then
            ...
            for index, status in ipairs( statusTable ) do
                ...
                icon.wimg = icon.wimg or vox.wimg.Simple( url, 'smooth mips' )
...
vox.hud:RegisterElement( 'vehicle', {
