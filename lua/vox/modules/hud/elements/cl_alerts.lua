...
local function addAlert( text )
    ...
    local data = {
        title = vox.utf8.upper( vox.lang:Get( 'message' ) ),
        text = text,
        duration = DURATION
    }
...
local function drawAlerts( element, client, scrW, scrH )
    ...
    surface.SetAlphaMultiplier( alert.alpha )
        ...
        render.SetScissorRect( x, y + h - lineH, x + w, y + h, true )
            hud.DrawRoundedBox( x, y, w, h, vox.LerpColor( .75, colorAccent, colorPrimary ) )
...
net.Receive( 'vox.hud::SendAlert', function()
    if ( vox.hud.IsElementEnabled( 'alerts' ) ) then
        addAlert( net.ReadString() )
    end
end )

hook.Add( 'vox.inconfig.Updated', 'vox.hud.ClearAlerts', function( id, old, new )
    if ( id and id == 'hud_display_alerts' ) then
        alerts = {}
    end
end )

vox.hud:RegisterElement( 'alerts', { drawFn = drawAlerts } )
