...
vox.hud.popups = vox.hud.popups or {}
...
local function movePopups()
    ...
    for _, popup in ipairs( vox.hud.popups ) do
        popup.animY = popup.animY or posY

        popup:SetVisible( true )

        vox.anim.Create( popup, .2, {
            index = 2,
            target = { animY = posY },
            easing = 'inOutSine',
            think = function( _, panel )
...
local function createPopup( title, desc, duration, acceptCallback, denyCallback, onRemove )
    ...
    popup.OnRemove = function( panel )
        for index, popup2 in ipairs( vox.hud.popups ) do
            if ( panel == popup2 ) then
                table.remove( vox.hud.popups, index )
                break
            end
        end
    ...
    table.insert( vox.hud.popups, popup )
    movePopups()

    startFadeAnimation( popup, 1 )

    local lblTitle = popup:Add( 'DLabel' )
    lblTitle:SetText( vox.utf8.upper( title ) )
    ...
    local btnAccept = conButtons:Add( 'vox.Button' )
    ...
    btnAccept:SetColorHover( vox.OffsetColor( btnAccept:GetColorIdle(), -20 ) )
    btnAccept:SetGradientColor( vox.OffsetColor( btnAccept:GetColorIdle(), -20 ) )
    ...
    local btnDeny = conButtons:Add( 'vox.Button' )
    ...
    btnDeny:SetColorHover( vox.OffsetColor( hud:GetColor( 'tertiary' ), -10 ) )
    btnDeny:SetGradientColor( vox.OffsetColor( btnDeny:GetColorIdle(), -20 ) )
    ...
    hook.Add( 'vox.hud.OnChangedTheme', popup, function( this )
        ...
        this.btnAccept:SetColorHover( vox.OffsetColor( this.btnAccept:GetColorIdle(), -20 ) )
        this.btnAccept:SetGradientColor( vox.OffsetColor( this.btnAccept:GetColorIdle(), -20 ) )

        this.btnDeny:SetColorIdle( hud:GetColor( 'tertiary' ) )
        this.btnDeny:SetColorHover( vox.OffsetColor( hud:GetColor( 'negative' ), -20 ) )
        this.btnDeny:SetGradientColor( vox.OffsetColor( this.btnDeny:GetColorIdle(), -20 ) )
    end )
...
vox.hud.OverrideGamemode( 'vox.hud.OverrideVoteMenus', overrideDarkRP )
