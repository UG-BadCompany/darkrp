...
function vox.scoreboard:RegisterNameEffect(id, func)
    table.insert(vox.scoreboard.nameEffects, {
        id = id,
        func = func,
...
function vox.scoreboard:FindNameEffect(id)
...
do
    ...
    -- Common (colorized)
    vox.scoreboard:RegisterNameEffect('default', function(text, x, y, color, ax, ay)
    ...
    -- Glow (colorized)
    vox.scoreboard:RegisterNameEffect('glow', function(text, x, y, color, ax, ay)
        SimpleText(text, fontCommon, x + SHADOW_DISTANCE, y + SHADOW_DISTANCE, COLOR_SHADOW, ax, ay)
    ...
    -- Rainbow
    vox.scoreboard:RegisterNameEffect('rainbow', function(text, x, y, color, ax, ay)
        local lastX = x
        local counter = 0
    ...
    -- Wavy Dual
    vox.scoreboard:RegisterNameEffect('wavy_dual', function(text, x, y, color, ax, ay)
        local lastX = x
        local counter = 0
        ...
        for _, code in utf8.codes(text) do
            ...
            local color2 = vox.ColorEditHSV(color1, (h + 90) % 360)
            local color = vox.LerpColor(fraction, color1, color2)
    ...
    -- Gradient
    vox.scoreboard:RegisterNameEffect('gradient_invert', function(text, x, y, color, ax, ay, realX, realY)
        local color1 = color
        local h, s, v = ColorToHSV(color1)
        local color2 = vox.ColorEditHSV(color1, (h + 180) % 360)
    ...
    -- Scanning
    vox.scoreboard:RegisterNameEffect('scanning_vertical', function(text, x, y, color, ax, ay, realX, realY)
        SimpleText(text, fontCommon, x + SHADOW_DISTANCE, y + SHADOW_DISTANCE, COLOR_SHADOW, ax, ay)
    ...
    vox.scoreboard:RegisterNameEffect('scanning_horizontal', function(text, x, y, color, ax, ay, realX, realY)
        SimpleText(text, fontCommon, x + SHADOW_DISTANCE, y + SHADOW_DISTANCE, COLOR_SHADOW, ax, ay)
        SimpleText(text, fontGlow, x, y, ColorAlpha(color, 30), ax, ay)
        local textW, textH = SimpleText(text, fontCommon, x, y, color_black, ax, ay)
...
net.Receive('vox.scoreboard:SyncRanks', function(length)
    local rawData = net.ReadData(length)
    local parsedData = pon.decode(rawData)

    vox.scoreboard.ranks = parsedData
    vox.scoreboard:Print('Synchronized ranks.')

    hook.Run('vox.scoreboard.SyncedRanks')
end)
