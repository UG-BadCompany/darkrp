...
function PANEL:Init()
    self.m_Font = vox.Font('Comfortaa@16')
    self.columns = {}
end

function PANEL:InitColumns()
    local activeColumns = vox.scoreboard:GetActiveColumns()
...
function PANEL:CreateColumn(data, index)
    local column = self:Add('vox.Label')
    if (data.id == 'team' and DarkRP) then
        column:SetText(vox.lang:Get('scoreboard_col_job'))
    else
        column:SetText(vox.lang:Get(data.name))
    end
    ...
    if (self:GetHeader()) then
        ...
        if (data.icon) then
            local wimgIcon = vox.wimg.Simple(data.icon, 'smooth mips')
            column:SetText('')
            column.Paint = function(panel, w, h)
                local size = math.min(h, vox.ScaleTall(16))
...
vox.gui.Register('vox.Scoreboard.ColumnsRow', PANEL)
