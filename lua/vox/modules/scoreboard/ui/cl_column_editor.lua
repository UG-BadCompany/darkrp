...
local COLOR_BG = vox.ColorBetween(COLOR_PRIMARY, COLOR_SECONDARY)
local COLOR_NEGATIVE = vox:Config('colors.negative')

local WIMG_SAVE = vox.wimg.Simple('https://i.imgur.com/ycSNF3m.png', 'smooth mips')
local PANEL = {}
...
function PANEL:Init()
    ...
    hook.Add('vox.scoreboard.SyncedColumns', self, function(panel)
        panel.grid:Clear()
        panel.columns = {}
        panel.default = {}
        panel.changes = {}
...
function PANEL:AddSavePopup()
    ...
    self.confirmPopup.button:SetColorHover(vox.OffsetColor(COLOR_NEGATIVE, -20))
    self.confirmPopup.button.DoClick = function()
        local changes = self:GetChanges()
        if (changes) then
            local amount = table.Count(changes)
            if (amount > 0) then
                net.Start('vox.scoreboard:SetColumns')
...
function PANEL:Think()
    if ((self.nextThink or 0) <= CurTime()) then
        ...
        if ((confirmPopup.targetAnim or -1) ~= anim) then
            ...
            vox.anim.Create(confirmPopup, .33, {
                index = 2,
                easing = 'inQuad',
                target = {
                    anim = anim
...
function PANEL:LoadColumns()
    for index = 1, vox.scoreboard.columnsMaxAmount do
...
function PANEL:AddColumn(index)
    ...
    lblTitle:SetText(vox.lang:Get('column') .. ' #' .. index)
    lblTitle:Dock(TOP)
    lblTitle:Font('Comfortaa Bold@16')
    lblTitle:SizeToContentsY()
    lblTitle:DockMargin(0, 0, 0, vox.ScaleTall(5))

    local comboType = panel:Add('vox.ComboBox')
    comboType:Dock(FILL)
    comboType:AddOption(vox.lang:Get('scoreboard_col_none'), 'none')
    ...
    for columnID, columnData in pairs(vox.scoreboard.columns) do
        if (not columnData.customCheck or columnData.customCheck()) then
            comboType:AddOption(vox.lang:Get(columnData.name), columnID)
        end
    end

    local currentID = vox.scoreboard.columnsCustomizable[index] or vox.scoreboard.columnsDefault[index] or 'none'
...
vox.gui.Register('vox.scoreboard.ColumnEditor', PANEL)

-- if (IsValid(DebugPanel)) then
--     DebugPanel:Remove()
-- end

-- DebugPanel = vox.scoreboard.OpenAdminSettings(3)
