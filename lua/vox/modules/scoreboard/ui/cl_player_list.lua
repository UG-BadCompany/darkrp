...
local WIMG_ARROW = vox.wimg.Simple('https://i.imgur.com/oOoWQAG.png', 'smooth mips')

local function parseString(str)
    return vox.utf8.lower(str)
end
...
local function processSearch(prompt, parentList)
    ...
    for _, category in ipairs(categories) do
        if (category.ClassName ~= 'vox.Category') then continue end
...
function PANEL:Init()
    ...
    self.search.OnMousePressed = function(panel)
        vox.scoreboard.Frame:SetKeyboardInputEnabled(true)
        vox.scoreboard.Frame:ShowCloseButton(true)
        vox.scoreboard.Frame.closeDisabled = true

        panel.textEntry:SetMouseInputEnabled(true)
        panel.textEntry:RequestFocus()
    end

    self.columns = self.toolbar:Add('vox.Scoreboard.ColumnsRow')
    self.columns:Dock(FILL)
    self.columns:DockMargin(toolbarPadding, 0, 0, 0)
    self.columns:SetFont(vox.Font('Comfortaa Bold@16'))
    ...
    self.list:SetSpace(vox.ScaleTall(1))
...
do
    function PANEL:FetchCategory(ply)
        if (DarkRP and vox.scoreboard:GetOptionValue('group_teams')) then
        ...
        if (vox.scoreboard.IsTTT()) then
            local _, name, color = vox.scoreboard.GetTeamTTT(ply)
            return name, color
        end
...
function PANEL:LoadPlayers()
    ...
    for _, ply in ipairs(player.GetAll()) do
        local bShouldHide = hook.Run('vox.scoreboard.ShouldHidePlayer', ply) -- for possible custom integrations
    ...
    for _, ply in ipairs(players) do
        ...
        if (not foundIndex) then
            local panel = self:AddCategory(vox.utf8.upper(catName), catColor) -- utf-8 support
...
function PANEL:AddSorting(panel)
    for index, column in ipairs(panel.columns) do
        ...
        column.PaintOver = function(panel, w, h)
            ...
            local size = math.min(h, vox.ScaleTall(12))
            local xPosOverride
...
function PANEL:AddPlayer(ply, parentCategory)
    ...
    local cat = parentCategory:Add('vox.Category')
    cat:SetCategoryHeight(self.playerLineHeight)
    cat:SetTall(cat:GetCategoryHeight())
    cat:SetInset(vox.ScaleTall(10))
    cat:SetSpace(0)
    cat:DockMargin(0, 0, 0, vox.ScaleTall(2.5))
    cat:Dock(TOP)

    -- keep size up to date
    cat:InjectEventHandler('PerformLayout')
    cat:On('PerformLayout', function(panel)
        if (panel.voxAnims and panel.voxAnims[1]) then -- fixes unpleasant view glitch
    ...
    cat.canvas.Paint = function(panel, w, h)
        draw.RoundedBoxEx(8, 0, 0, w, h, vox.LerpColor(.025, COLOR_PRIMARY, color_black), false, false, true, true)
    end

    local profile = cat:Add('vox.scoreboard.PlayerInspector')
    profile:Dock(TOP)
    profile:SetupPlayer(ply)

    -- create actual playerLine
    local playerLine = cat.button:Add('vox.Scoreboard.PlayerLine')
...
function PANEL:AddCategory(name, color)
    local cat = self.list:Add('vox.Category')
    cat:Dock(TOP)
    cat:SetTitle(name)
    cat:SetSpace(vox.ScaleTall(5))
    cat:SetInset(0)
    cat:SetTextMargin(self.toolbarPadding + 2)
    cat:DockMargin(0, 0, 0, vox.ScaleTall(5))
...
local wimgCopy = vox.wimg.Simple('https://i.imgur.com/OolNq4H.png', 'smooth mips')
...
function PANEL:Init()
    self.buttonHeight = vox.ScaleTall(20)
    ...
    self._avatar.PerformLayout = function(panel, w, h)
        ...
        panel.poly = vox.CalculateRoundedBox(avatarRoundness, x, y, size, size)
    end
    self._avatar.Paint = function(panel, w, h)
        ...
        vox.DrawWithPolyMask(panel.poly, function()
    ...
    self.buttons = self:Add('vox.Grid')
    self.buttons:SetColumnCount(3)
    self.buttons:SetSpace(vox.ScaleTall(5))

    self.info = self:Add('Panel')

    for _, buttonTable in ipairs(vox.scoreboard.Buttons) do
        local getVisible = buttonTable.getVisible

        if (getVisible and getVisible(LocalPlayer()) == false) then continue end

        self:AddButton(vox.lang:Get('scoreboard_btn_' .. buttonTable.name), function()
...
function PANEL:PerformLayout(w, h)
    ...
    self._avatar:DockMargin(0, 0, vox.ScaleTall(10), 0)
...
function PANEL:SetupPlayer(ply)
    ...
    if (yourself) then
        formattedName = formattedName .. ' (' .. vox.lang:Get('you') .. ')'
        setTextColor(self.labels.name, COLOR_GOLD)
    elseif (ply:GetFriendStatus() == 'friend') then
        formattedName = formattedName .. ' (' .. vox.lang:Get('friend') .. ')'
...
function PANEL:AddLabel(id, title)
    ...
    row:SetTall(vox.ScaleTall(20))
    row:Dock(TOP)
    row:DockMargin(0, 0, 0, 0)

    local lblTitle = row:Add('vox.Label')
    ...
    local lblValue = row:Add('vox.Label')
    ...
    lblValue.DoClick = function(panel)
        ...
        notification.AddLegacy(vox.lang:Get('copied_clipboard'), 0, 5)
    end
...
function PANEL:AddButton(text, func)
    local button = self.buttons:Add('vox.Button')
    button:SetText(text)
