...
vox.scoreboard.columns = {}

--[[------------------------------
Default ones
--------------------------------]]
vox.scoreboard:RegisterColumn('team', {
    getValue = function(client)
        if (vox.scoreboard.IsTTT()) then
            return select(2, vox.scoreboard.GetTeamTTT(client))
        end
    ...
    getColor = function(client)
        if (vox.scoreboard.IsTTT()) then
            return select(3, vox.scoreboard.GetTeamTTT(client))
        end

        return vox.scoreboard.ConvertTeamColor(team.GetColor(client:Team()))
    end
})

vox.scoreboard:RegisterColumn('rank', {
    getValue = function(client)
        local rank = client:GetUserGroup()
        local data = vox.scoreboard:GetRankData(rank)
        if (data) then
            return data.name
        end
        return rank
...
vox.scoreboard:RegisterColumn('health', {
    getValue = function(client)
...
DarkRP
--------------------------------]]
vox.scoreboard:RegisterColumn('money', {
    getValue = function(client)
...
TTT
--------------------------------]]
vox.scoreboard:RegisterColumn('karma', {
    getValue = function(client)
...
do
    ...
    vox.scoreboard:RegisterColumn('level', {
        getValue = function(client)
...
do
    ...
    vox.scoreboard:RegisterColumn('playtime', {
        getValue = function(client)
            for _, data in ipairs(handlers) do
                if (data.valid()) then
                    local value = data.func(client)
...
-- Ashop
vox.scoreboard:RegisterColumn('ashop_badges', {
    getValue = function(client)
        ...
        for slot, data in pairs(badges) do
            ...
            badge.Paint = function(panel, w, h)
                xpcall(function()
                    ...
                    vox.scoreboard:PrintError('(AShop) Error during badge display: #', errText)
                end)
            end
        end
    end,
...
-- Brick's Gangs
vox.scoreboard:RegisterColumn('gang', {
    getValue = function(client)
        local gangID = client:GetGangID()
        local gangName = vox.scoreboard.GetBricksGangName(gangID)
        return gangName
    end,
    customCheck = function()
...
-- VoidFactions
vox.scoreboard:RegisterColumn('faction', {
    getValue = function(client)
