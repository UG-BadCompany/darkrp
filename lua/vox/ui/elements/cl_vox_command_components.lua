...
vox.command.colors = vox.command.colors or {
    base = Color(5, 8, 16), glass = Color(10, 16, 29), panel = Color(14, 23, 39), raised = Color(21, 33, 54),
    accent = Color(0, 188, 255), blueGlow = Color(70, 135, 255), text = Color(242, 247, 255), muted = Color(139, 154, 176),
    money = Color(72, 221, 142), danger = Color(255, 78, 104), armor = Color(88, 166, 255), amber = Color(255, 190, 88)
}

function vox.CommandPalette()
    local p = table.Copy(vox.command.colors)
    local theme = (vox.GetThemeColors and vox.GetThemeColors()) or {}
    p.accent = safeColor(theme.accent, p.accent); p.blueGlow = safeColor(theme.accent, p.blueGlow)
...
function vox.DrawVoxGlass(x, y, w, h, state)
    state = state or {}; local p = vox.CommandPalette(); local a = safeColor(state.accent, p.accent); local r = state.radius or 18
    ...
    if vox.DrawMatGradient then
        ...
        vox.DrawMatGradient(x, y, w, h, BOTTOM, ColorAlpha(p.blueGlow, 15))
    end
...
function vox.DrawVoxCommandCard(x, y, w, h, state)
    state = state or {}; local p = vox.CommandPalette(); local a = safeColor(state.accent, p.accent); local lift = state.hovered and 3 or 0
...
local Root = {}
function Root:Init() self:SetAlpha(0); self:AlphaTo(255,.18,0); self.accent = vox.CommandPalette().accent end
function Root:Paint(w,h) if vox.DrawBlurExpensive then vox.DrawBlurExpensive(self, 8) end; vox.DrawVoxGlass(0,0,w,h,{radius=24,alpha=244,accent=self.accent,shadow=100}); draw.SimpleText('VOX', font('Comfortaa Bold@22'), 28, 24, vox.CommandPalette().text) end
register('VoxRootFrame','vox.Frame',Root)

local Glass = {}; function Glass:Init() self.accent = vox.CommandPalette().accent end; function Glass:Paint(w,h) vox.DrawVoxGlass(0,0,w,h,{radius=self.radius or 18,accent=self.accent}) end; register('VoxGlassFrame','Panel',Glass); register('VoxTopBar','VoxGlassFrame',{}); register('VoxInfoCard','VoxGlassFrame',{})
local Card = {}; function Card:Init() self.hover=0; self.accent=vox.CommandPalette().accent end; function Card:Think() self.hover=Lerp(FrameTime()*12,self.hover,self:IsHovered() and 1 or 0) end; function Card:Paint(w,h) vox.DrawVoxCommandCard(0,0,w,h,{accent=self.accent,hovered=self.hover>.1,radius=16}) end
...
local Button = {}; function Button:Init() self:SetTextColor(vox.CommandPalette().text); self:SetFont(font('Comfortaa Bold@16')); self.hover=0 end; function Button:Think() self.hover=Lerp(FrameTime()*14,self.hover,self:IsHovered() and 1 or 0) end; function Button:Paint(w,h) local p=vox.CommandPalette(); vox.DrawVoxGlass(0,0,w,h,{radius=12,accent=p.accent,alpha=205+35*self.hover,glow=20+35*self.hover}); return false end
register('VoxActionButton','DButton',Button); register('VoxIconButton','DButton',Button); register('VoxSidebarItem','DButton',Button)

local Entry = {}; function Entry:Init() self:SetTall(42); self:SetFont(font('Comfortaa@15')); self:SetTextColor(vox.CommandPalette().text); self:SetPlaceholderColor(vox.CommandPalette().muted) end; function Entry:Paint(w,h) vox.DrawVoxGlass(0,0,w,h,{radius=13,alpha=218}); self:DrawTextEntryText(vox.CommandPalette().text,vox.CommandPalette().accent,vox.CommandPalette().text) end
...
function vox.DrawVoxCard( x, y, w, h, colors, state )
    ...
    vox.DrawVoxCommandCard( x, y, w, h, { radius = state.radius or 16, accent = state.accent or colors.accent, base = colors.primary, hovered = state.hovered } )
end

function vox.DrawVoxRow( x, y, w, h, colors, state )
    colors = colors or {}; state = state or {}
    draw.RoundedBox( state.radius or 12, x, y, w, h, ColorAlpha( colors.secondary or vox.CommandPalette().panel, state.alpha or 218 ) )
    if state.hovered or state.selected then
        local accent = state.accent or colors.accent or vox.CommandPalette().accent
