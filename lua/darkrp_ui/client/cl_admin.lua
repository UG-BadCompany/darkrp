...
local reasonActions={kick=true,warn=true,ban=true,jail=true}
local durationActions={ban=true,jail=true}
local destructiveActions={stripweapons=true,slay=true,kick=true,ban=true,jail=true}
local function requestActionData(action, label, target, cb)
    local function askDuration(data)
        if not durationActions[action] then cb(data); return end
        Derma_StringRequest(label.." duration", "Duration in seconds for "..target:Nick(), "3600", function(v) data.duration=tonumber(v) or 0; cb(data) end)
    end
    local data={reason="Requested from DarkRPUI", duration=0}
    if reasonActions[action] then
        Derma_StringRequest(label.." reason", "Enter a moderation reason for "..target:Nick(), "", function(v) data.reason=v or ""; askDuration(data) end)
    else askDuration(data) end
end
...
function DarkRPUI.Admin.Open(target)
    ...
    local f=vgui.Create("DFrame"); DarkRPUI.Admin.Frame=f; local fw,fh=DarkRPUI.Layout.SizeForScreen(900,640); f:SetSize(fw,fh); f:Center(); DarkRPUI.Layout.ClampPanel(f,true); f:SetTitle(""); f:ShowCloseButton(false); f:SetDraggable(false); f:MakePopup(); DarkRPUI.UI.AnimatePanelIn(f)
    ...
    local left=vgui.Create("DPanel",f); left:SetPos(22,92); left:SetSize(math.min(350, f:GetWide()*0.44),f:GetTall()-114); left.Paint=function(_,w,h) DarkRPUI.UI.ShadowedBox(18,0,0,w,h,DarkRPUI.Color("panel"),DarkRPUI.Color("border"),90) end
    local searchHolder, search = DarkRPUI.UI.PremiumSearch(left,"Search name, SteamID, job...",function(v) searchText=string.lower(v or ""); if IsValid(left.List) and left.Rebuild then left.Rebuild() end end); searchHolder:SetPos(14,14); searchHolder:SetSize(left:GetWide()-28,44)
    local list=vgui.Create("DScrollPanel",left); left.List=list; list:SetPos(14,70); list:SetSize(left:GetWide()-28,left:GetTall()-84); DarkRPUI.UI.StyleScrollbar(list)
    local detail=vgui.Create("DPanel",f); detail:SetPos(left:GetX()+left:GetWide()+18,92); detail:SetSize(f:GetWide()-left:GetWide()-62,f:GetTall()-114); detail.Paint=function(_,w,h)
    ...
    local grid=vgui.Create("DIconLayout",detail); grid:SetPos(22,126); grid:SetSize(detail:GetWide()-44,detail:GetTall()-150); grid:SetSpaceX(10); grid:SetSpaceY(10)
    local function rebuildActions()
        grid:Clear(); for _,a in ipairs(actions) do if canLocal(a.id) then local b=vgui.Create("DButton",grid); b:SetSize(147,42); b:SetText(a.label); DarkRPUI.UI.StyleButton(b,DarkRPUI.Color(a.tone)); DarkRPUI.UI.AttachTooltip(b,"Run "..a.label.." on the selected player."); b.DoClick=function() if not IsValid(selected) then DarkRPUI.Notify("warning","No player selected","Choose a player first."); return end; requestActionData(a.id,a.label,selected,function(data) local body=(destructiveActions[a.id] and "This is destructive. " or "").."Run "..a.label.." on "..selected:Nick().."?"; DarkRPUI.UI.Confirm("Confirm admin action",body,"Run","Cancel",function(ok) if ok then DarkRPUI.Admin.Send(a.id,selected,data) end end) end) end end end
    end
