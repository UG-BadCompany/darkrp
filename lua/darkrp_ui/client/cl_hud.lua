...
hook.Add("HUDPaint", "DarkRPUI.HUD.Paint", function()
    ...
    local compact=DarkRPUI.Settings and DarkRPUI.Settings.compact; local show=DarkRPUI.Settings or {}; local resScale=(DarkRPUI.Config.LowResolutionScale ~= false) and math.Clamp(ScrW()/1280, .78, 1) or 1; local s=(DarkRPUI.Settings and DarkRPUI.Settings.hud_scale or 1) * (compact and .9 or 1) * resScale; local pad=DarkRPUI.Util.Scale(22*s); local w=DarkRPUI.Util.Scale((compact and 330 or 450)*s); local h=DarkRPUI.Util.Scale((compact and 104 or 154)*s); local pos=show.hud_position or "bottom-left"; local x=(string.find(pos,"right",1,true) and (ScrW()-w-pad) or pad); local y=(string.find(pos,"top",1,true) and pad or (ScrH()-h-pad))
    ...
    if not compact and show.show_agenda ~= false and DarkRPUI.Config.HUD.showAgenda then local agenda=DarkRPUI.Util.DarkRPVar(ply,"agenda",nil); if agenda then card(ScrW()-pad-330,92,330,96,"AGENDA",tostring(agenda),DarkRPUI.Color("accent"),1,"▣") end end
