...
function DarkRPUI.SettingsPanel(parent)
    ...
    local function toggle(label,key) local b=vgui.Create("DButton",p); b:SetPos(0,y); b:SetSize(320,42); b:SetText(""); b.Paint=function(s,w,h) s.Hover=DarkRPUI.UI.LerpValue(s.Hover or 0, s:IsHovered() and 1 or 0, 12); DarkRPUI.UI.RoundedBox(10,0,0,w,h,DarkRPUI.LerpColor(s.Hover,DarkRPUI.Color("card"),DarkRPUI.Color("cardHover"))); DarkRPUI.UI.Text(label,"DarkRPUI.Body",14,11); DarkRPUI.UI.Text(DarkRPUI.Settings[key] and "ON" or "OFF","DarkRPUI.Body",w-18,11,DarkRPUI.Settings[key] and DarkRPUI.Color("success") or DarkRPUI.Color("error"),TEXT_ALIGN_RIGHT) end; b.DoClick=function() DarkRPUI.Settings[key]=not DarkRPUI.Settings[key]; save() end; y=y+52 end
    ...
    local theme=vgui.Create("DComboBox",p); theme:SetPos(0,y); theme:SetSize(320,42); theme:SetValue(DarkRPUI.Theme(DarkRPUI.ActiveTheme).name); for id,t in pairs(DarkRPUI.Themes or {}) do theme:AddChoice(t.name,id) end; theme.OnSelect=function(_,_,_,id) DarkRPUI.Settings.theme=id; if DarkRPUI.SetTheme then DarkRPUI.SetTheme(id) end; save() end
end
